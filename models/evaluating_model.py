...
parser.add_argument('-o','--model_dir', help='model dir with checkpoints', required=True)
...
TAGS = ['ADDRESS', 'CITY', 'COUNTRY', 'ORG', 'POSTALCODE', 'REGION', 'SUB']
DEFAULT_CRITERIA = {
    'ent_type': 'TYPE (Overlap with matching types)',
    'strict': 'STRICT (Exact string with matching types)'
}

def display_results_nervaluate(results_by_tag, criteria_to_consider=DEFAULT_CRITERIA):
  dfs = {}
  for entity_type, results in results_by_tag.items():
      for criterion, metrics in results.items():
          if criterion in criteria_to_consider:
            df = pd.DataFrame(metrics, index=[entity_type])
            if criterion not in dfs:
                dfs[criterion] = df
            else:
                dfs[criterion] = pd.concat([dfs[criterion], df])
  # Print DataFrames for each evaluation criterion
  for criterion, df in dfs.items():
      print(f"Evaluation: {criteria_to_consider[criterion]}\n")
      display(df)
      print("\n")

def get_results_nervaluate(results_by_tag, criteria_to_consider=DEFAULT_CRITERIA):
  dfs = {}
  for entity_type, results in results_by_tag.items():
      for criterion, metrics in results.items():
          if criterion in criteria_to_consider:
...
for i, checkpoint_path in enumerate(glob.glob(model_dir)):
    epoch = i+1
    model = AutoModelForTokenClassification.from_pretrained(checkpoint_path, ignore_mismatched_sizes=True, \
    ...
    train_testvalid = example_ds.train_test_split(test_size=0.2, seed=42)
    # Split the 10% test + valid in half test, half valid
    test_valid = train_testvalid['test'].train_test_split(test_size=0.5, seed=42)
    # gather everyone if you want to have a single DatasetDict
    train_test_valid_dataset = DatasetDict({
    ...
    for set_name in ['test','val']:
        ...
        predictions = trainer.predict(train_test_valid_dataset[set_name])
        predictions_labels = np.argmax(predictions.predictions, axis=-1)

        all_predictions = []
        all_labels = []

        all_predictions_by_instance = []
        all_labels_by_instance = []

        labels = train_test_valid_dataset[set_name]['labels']
        ...
        for prediction, label in zip(predictions, labels):
            ...
            for predicted_idx, label_idx in zip(prediction, label):
                if label_idx == -100:
                    continue
                predictions_instance.append(dm.id2label[predicted_idx])
                labels_instance.append(dm.id2label[label_idx])
                all_predictions.append(dm.id2label[predicted_idx])
                all_labels.append(dm.id2label[label_idx])
            all_predictions_by_instance.append(predictions_instance)
            all_labels_by_instance.append(labels_instance)

        # Evaluate results.
        evaluator = Evaluator(all_labels_by_instance, all_predictions_by_instance, tags=TAGS, loader="list")
        results, results_by_tag = evaluator.evaluate()

        res = seqeval.compute(predictions=[all_predictions], references=[all_labels], mode='strict')
        pd.DataFrame(res).transpose()

        df_type = get_results_nervaluate(results_by_tag)['ent_type']
        df_strict = get_results_nervaluate(results_by_tag)['strict']
        df_results = pd.DataFrame(results)
        df_type.to_csv(f'{output_dir}/{model_name}_{epoch}_type_{set_name}.csv')
        df_strict.to_csv(f'{output_dir}/{model_name}_{epoch}_strict_{set_name}.csv')
        df_results.to_csv(f'{output_dir}/{model_name}_{epoch}_results_{set_name}.csv')
