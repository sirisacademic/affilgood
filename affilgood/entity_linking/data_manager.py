...
class DataManager:
    ...
    def download_file(self, url_or_bucket, key_or_path, local_path, is_s3=False):
        """
        ...
        # Check for omission
        if self.should_skip_s3_key(key_or_path):
    ...
    def sync_s3_to_local(self, bucket, prefix, local_dir):
        """Syncs S3 objects to a local directory, skipping based on conditions."""
        for obj in self.list_s3_objects(bucket, prefix):
            s3_key = obj["Key"]
            relative_path = s3_key[len(prefix):].lstrip("/")
            local_path = os.path.join(local_dir, relative_path)
            if os.path.exists(local_path):
                print(f"Skipping existing {s3_key}")
                continue
            self.download_file(bucket, s3_key, local_path, is_s3=True)

    def download_s2aff_data(self):
        """Downloads the S2AFF data from the specified S3 bucket."""
        bucket_name = "ai2-s2-research-public"
        prefix = "s2aff-release"
        self.sync_s3_to_local(bucket_name, prefix, self.data_s2aff_path)
        ...
        for key, path in PATHS.items():
            if path.startswith("http"):
                ...
                if not os.path.exists(local_path):
                    ...
                    self.download_file(path, os.path.basename(path), local_path)
    ...
    def get_latest_ror(self, ror_dump_path=ROR_DUMP_PATH):
        ...
        try:
            ...
            self.download_file(download_url, None, file_path)
    ...
    def update_openalex_works_counts(self):
        """Updates OpenAlex work counts using only the latest available data from S3."""
        if self.should_update_openalex():
            ...
            # Find the latest file based on the 'updated_date' prefix
            latest_file = None
            latest_date = None
            ...
            for page in pages:
                for obj in page.get("Contents", []):
                    key = obj["Key"]
                    if key.endswith(".gz") and "updated_date=" in key:
                        # Extract the date from the key
                        date_str = key.split("updated_date=")[1].split("/")[0]
            ...
            if not latest_file:
                print("No suitable file found in the S3 bucket.")
                return
